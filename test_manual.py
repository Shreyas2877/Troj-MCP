...
def test_mcp_protocol():
    ...
    try:
        ...
        if response_line:
            ...
            print(
                f"✅ Initialize: {response.get('result', {}).get('serverInfo', {}).get('name', 'Unknown')}"
            )
        ...
        if response_line:
            response = json.loads(response_line.strip())
            result = (
                response.get("result", {}).get("content", [{}])[0].get("text", "Error")
            )
        ...
        call_request = {
            "jsonrpc": "2.0",
            "method": "tools/call",
            "params": {"name": "multiply_numbers", "arguments": {"a": 4, "b": 7}},
            "id": 4,
        }
